...
public class ShapelessArcaneRecipe implements IArcaneRecipe {
    ...
    public boolean isSpecial() {
        ...
        // ingredients"), which removed every arcane workbench recipe from the
        // recipe map on both the client and the server. The workbench grid is a
        // mod station, not a vanilla crafting grid, so mark it special: it stays
        // in the recipe map (station matching, JEI and the Thaumonomicon all
        // resolve it) but is hidden from the vanilla recipe book.
        return true;
    }
