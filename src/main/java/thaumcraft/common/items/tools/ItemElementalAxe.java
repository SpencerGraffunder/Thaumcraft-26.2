    ...
    public void onUseTick(Level level, LivingEntity player, ItemStack stack, int remainingUseDuration) {
        ...
        // Particle effects
                if (level.isClientSide()) {
                    level.addParticle(net.minecraft.core.particles.ParticleTypes.FLAME, player.getX(), player.getY() + 0.5, player.getZ(), 0, 0.1, 0);
                }
        // Sound effects
                if (!level.isClientSide() && level.getRandom().nextFloat() < 0.1f) {
                    level.playSound(null, player.getX(), player.getY(), player.getZ(), net.minecraft.sounds.SoundEvents.FIRE_AMBIENT, net.minecraft.sounds.SoundSource.NEUTRAL, 0.3f, 1.0f);
                }
    }
